	...
	class Ult_Content_Box {
		...
		function ult_content_box_callback($atts, $content = null){
		  extract( shortcode_atts( array(
		  		...
		  		switch ($bg_type) {
		  			...
		  			case 'bg_image': 	if($bg_image != '') {
											$img = wp_get_attachment_image_src( $bg_image, 'large');
										 	$style .= "background:url('".$img[0]."') ".$bg_repeat." scroll ".$bg_position." / ".$bg_size." rgba(0, 0, 0, 0);";
									  	}
					break;
		  		}
		  	}
		  	...
		  	/* 	box shadow 	*/
			if($box_shadow!='' /*&& $box_shadow_color!=''*/) {
				$style .= apply_filters('Ultimate_GetBoxShadow', $box_shadow, 'css');
				/*$arr = apply_filters('Ultimate_GetBoxShadow', $box_shadow, 'array');
				if(is_array($arr) && !empty($arr)) {
					$nr['color'] = $box_shadow_color; 			//	Update color
					array_splice( $arr, 4, 0, $nr ); 			//  Set color position
					$shadow = implode(' ', $arr); 				//	Convert to string for CSS styling
					$style .= 'box-shadow:'.$shadow.';';
				}*/
			}

		  	/* 	box shadow - {HOVER} 	*/
			if($hover_box_shadow!='' /*&& $box_hover_shadow_color!=''*/) {
				...
				/*$arr = apply_filters('Ultimate_GetBoxShadow', $hover_box_shadow, 'array');
				if(is_array($arr) && !empty($arr)) {
					$nr['color'] = $box_hover_shadow_color; 			//	Update color
					array_splice( $arr, 4, 0, $nr ); 			//  Set color position
					$shadow = implode(' ', $arr); 				//	Convert to string for CSS styling
					$hover .= ' data-hover_box_shadow="'.$shadow.'" ';
				}*/
			}
		...
		function ult_content_box_scripts() {
		  wp_register_style( 'ult_content_box_css', plugins_url('../assets/min-css/content-box.min.css',__FILE__) );
		  wp_register_script('ult_content_box_js', plugins_url('../assets/min-js/content-box.min.js',__FILE__) , array('jquery'), ULTIMATE_VERSION, true);
		}
	}
